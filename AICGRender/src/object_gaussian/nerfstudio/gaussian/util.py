...
from ...nerfstudio.gaussian.scene.cameras import Camera
def get_render_settings(w, h, intrinsics, w2c, near=0.01, far=100, sh_degree=3):
    """
    ...
    cam_ino=dict()
    cam_ino['image_height']=h
    cam_ino['image_width']=w
    cam_ino['tanfovx']=w / (2 * fx)
    cam_ino['tanfovy']=h / (2 * fy)
    cam_ino['bg']=torch.tensor([0, 0, 0], device='cuda').float()
    cam_ino['scale_modifier']=1.0
    cam_ino['viewmatrix']=viewmatrix
    cam_ino['projmatrix']=full_proj_matrix
    cam_ino['sh_degree']=sh_degree
    cam_ino['campos']=cam_center
    cam_ino['prefiltered']=False
    cam_ino['debug']=False
    return cam_ino
...
def render_gaussian_model(gaussian_model, render_settings,
    ...
    return {"color": color, "depth": depth, "radii": radii, "means2D": means2D, "alpha": alpha,"visibility_filter" : radii > 0,"viewspace_points": means2D}
