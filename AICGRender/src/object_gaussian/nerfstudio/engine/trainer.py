    ...
    def train(self) -> None:
        ...
        with TimeWriter(writer, EventName.TOTAL_TRAIN_TIME):
            ...
            for step in range(self._start_step, self._start_step + num_iterations):
                ...
                #     new_content = f"step {step} Train Loss = {loss}, rgb_loss {loss_dict['rgb_loss'].item()} , eikonal_loss {loss_dict['eikonal_loss'].item()}, interlevel_loss {loss_dict['interlevel_loss'].item()}\n {loss_dict}\n" 
  
                #     # 使用'a'模式打开文件以追加内容  
                #     with open('loss.txt', 'a') as file:  
                #         file.write(new_content)  # 写入新内容  
                    # print("\n Train Loss = ",loss," rgb ",loss_dict["rgb_loss"],"\n")
                    # print("\n Train Loss rgb = ",loss_dict["rgb_loss"],"\n")
