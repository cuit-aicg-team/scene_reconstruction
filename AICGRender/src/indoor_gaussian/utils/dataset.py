...
class OurInDataset(BaseDataset):
    def __init__(self, args, path, config,cam_intrinsics):
        super().__init__(args, path, config)
        # calibration = config["Dataset"]["Calibration"]
        # Camera prameters
        self.fx = cam_intrinsics["params"][0]
        self.fy = cam_intrinsics["params"][1]
        self.cx = cam_intrinsics["params"][2]
        self.cy = cam_intrinsics["params"][3]
        ...
        # distortion parameters
        self.disorted = cam_intrinsics["distorted"]
        self.dist_coeffs = np.array(
            [
                cam_intrinsics["k1"],
                cam_intrinsics["k2"],
        ...
        # depth parameters
        self.has_depth = True if "depth_scale" in cam_intrinsics.keys() else False
        self.depth_scale = cam_intrinsics["depth_scale"] if self.has_depth else None
...
class OurDataset(OurInDataset):
    def __init__(self, args, path, config,cam_intrinsics,cam_extrinsics):
        super().__init__(args, path, config,cam_intrinsics)
        dataset_path = config["Dataset"]["dataset_path"]
        parser = ReplicaParser(dataset_path)
        self.num_imgs = parser.n_img
        self.color_paths = parser.color_paths
...
def load_dataset(args, path, cam_intrinsics,cam_extrinsics,config):
    if config["Dataset"]["type"] == "tum":
        return TUMDataset(args, path, config)
    elif config["Dataset"]["type"] == "replica":
        return ReplicaDataset(args, path, config)
    elif config["Dataset"]["type"] == "our":
        return OurDataset(args, path, config,cam_intrinsics,cam_extrinsics)
